...
public class CreateTicketForCustomerUseCase extends
    ...
    public Output execute(final Input input) {
        final var ticket = Ticket.newTicket(EventTicketId.with(input.eventTicketId), EventId.with(input.eventId), CustomerId.with(input.customerId));

        this.ticketRepository.create(ticket);

        return new Output(ticket.ticketId().value());
    }
